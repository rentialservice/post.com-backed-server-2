import { Retweet, Tweet, User } from '../../sequelize.js';
import { addRetweetValidation } from '../../utils/validation.js';

export const addRetweet = async (req, res) => {
  // body -> {userId, tweetId}
  ...
  if (!created) {
    return res.status(403).json({ errors: 'Tweet is already retweeted by user' });
  }
...
export const removeRetweet = async (req, res) => {
  ...
  if (unRetweet === 0) {
    return res.status(403).json({ errors: 'Tweet is already unretweeted by user' });
  }
...
export const getTweetRetweets = async (req, res) => {
  // body -> {tweetId}
